...
use Amelaye\BioTools\Form\MeltingTemperatureType;
use Amelaye\BioTools\Form\MicroArrayDataAnalysisType;
...
class MinitoolsController extends AbstractController
{
    ...
    public function skewsAction(Request $request, SkewsManager $skewsManager)
    {
        ...
        if ($request->isMethod('POST') && $form->handleRequest($request)->isValid()) {
            ...
            $imageResult = basename($skewsManager->createImage(
                $sequence,
                $formData["window"],
                $formData["GC"],
                $formData["AT"],
    ...
    /**
     * @param   Request             $request
     * @param   FormulasManager     $formulasManager
     * @return  Response
     * @throws  \Exception
     */
    #[Route('/minitools/useful-formulas', name: 'useful_formulas')]
    public function usefulFormulasAction(Request $request, FormulasManager $formulasManager)
    {
        $result = null;
        $formula = null;

        $form = $this->createForm(FormulasType::class);
        ...
        if ($request->isMethod('POST') && $form->handleRequest($request)->isValid()) {
            ...
            $result = match ($formula) {
                ...
                "mw_ssdna" => $formulasManager->mwOfSsDNA($sequence),
                "pmol_ends_dsdna" => $formulasManager->pmolOfDsDNA($sequence, $quantity),
                "pmol_ends_ssdna" => $formulasManager->pmolOfSsDNA($sequence, $quantity),
                "microg_to_pmol_dsdna" => $formulasManager->microToPmolDsDNA($sequence, $quantity),
                "microg_to_pmol_ssdna" => $formulasManager->microToPmolSsDNA($sequence, $quantity),
                "pmol_to_microg_dsdna" => $formulasManager->pmolToMicroDsDNA($sequence, $quantity),
                "pmol_to_microg_ssdna" => $formulasManager->pmolToMicroSsDNA($sequence, $quantity),
                "mw_ssrna" => $formulasManager->mwOfSsRNA($sequence),
                "pmol_ends_ssrna" => $formulasManager->pmolOfSsRNA($sequence, $quantity),
                "pmol_to_microg_ssrna" => $formulasManager->pmolToMicroSsRNA($sequence, $quantity),
                "centi_to_fahren" => $formulasManager->centiToFahren($value),
                "fahren_to_centi" => $formulasManager->farhenToCenti($value),
                "mbar_to_mmhg" => $formulasManager->mbarToMmHg($value),
                "mbar_to_inchhg" => $formulasManager->mbarToInchHg($value),
                "mbar_to_psi" => $formulasManager->mbarToPsi($value),
                "mbar_to_atm" => $formulasManager->mbarToAtm($value),
                "mbar_to_kpa" => $formulasManager->mbarToKPa($value),
        ...
        return $this->render(
            'minitools/usefulFormulas.html.twig',
            [
                'form' => $form->createView(),
                'formula' => $formula,
                'result' => $result
